###############################################################################
# Supplementary Table 3. Characteristics of the study population, overall,
...
#
# Columns: Variable | Overall | Male | Female | p (sex) |
#          Age <40 years | Age 40-59 years | Age >=60 years | p (age)
#   p (sex): male vs female
#   p (age): global comparison across the three age groups
#
...
#
# Analytical population, data cleaning, indices, and outcomes are the same
# as in the Table 1 script: BMI >= 18.5 kg/m2, sex recorded as female or
# male, and metabolic health status resolvable under both Definition A and
# Definition B. Participants without valid age are included in the overall
# and sex columns but not in the age-group columns.
#
...
# Comparison across groups (Kruskal-Wallis test)
...
# == Additional indices (same as Table 1) ====================================
...
dat <- dat %>%
  mutate(
    ...
    AGE_GROUP = factor(AGE_GROUP, levels = c("lt40", "40_59", "ge60"))
  )

idx_m  <- dat$Sex == "Male"
idx_f  <- dat$Sex == "Female"
idx_a1 <- !is.na(dat$AGE_GROUP) & dat$AGE_GROUP == "lt40"
idx_a2 <- !is.na(dat$AGE_GROUP) & dat$AGE_GROUP == "40_59"
idx_a3 <- !is.na(dat$AGE_GROUP) & dat$AGE_GROUP == "ge60"
...
summarise_row <- function(var, label, type) {
  ...
  if (type == "cont") {
    fmt   <- fmt_mean_sd
    p_sex <- p_continuous(x, dat$Sex)
    p_age <- p_continuous(x, dat$AGE_GROUP)
  } else {
    fmt   <- fmt_n_pct
    p_sex <- p_categorical(x, dat$Sex)
    p_age <- p_categorical(x, dat$AGE_GROUP)
  }
  ...
  tibble(
    ...
    Female   = fmt(x[idx_f]),
    p_sex    = fmt_p(p_sex),
    Age_lt40 = fmt(x[idx_a1]),
    Age_4059 = fmt(x[idx_a2]),
    Age_ge60 = fmt(x[idx_a3]),
    p_age    = fmt_p(p_age)
  )
}
...
section_row <- function(section) {
  tibble(
    Variable = section, Overall = "", Male = "", Female = "", p_sex = "",
    Age_lt40 = "", Age_4059 = "", Age_ge60 = "", p_age = ""
  )
}
...
n_row <- tibble(
  Variable = "n",
  Overall  = fmt_n(nrow(dat)),
  Male     = fmt_n(sum(idx_m)),
  Female   = fmt_n(sum(idx_f)),
  p_sex    = "",
  Age_lt40 = fmt_n(sum(idx_a1)),
  Age_4059 = fmt_n(sum(idx_a2)),
  Age_ge60 = fmt_n(sum(idx_a3)),
  p_age    = ""
)

supp_table_3 <- bind_rows(
  n_row,
  bind_rows(lapply(unique(table_spec$Section), function(sec) {
...
names(supp_table_3) <- c(
  ...
  "p (sex)",
  paste0("Age <40 years\n(n=",        fmt_n(sum(idx_a1)), ")"),
  paste0("Age 40\u201359 years\n(n=", fmt_n(sum(idx_a2)), ")"),
  paste0("Age \u226560 years\n(n=",   fmt_n(sum(idx_a3)), ")"),
  "p (age)"
)

# Check: N should match Table 1 and Figures 1-5
cat("\nAnalytical population:", nrow(dat),
    "| without valid age:", sum(is.na(dat$AGE_GROUP)), "\n")

# == Output ====================================================================

print(supp_table_3, n = Inf, width = Inf)

supp_table_3_footnote <- paste0(
  "Data are mean (SD) or n (%). p values were obtained with the ",
  "Kruskal\u2013Wallis test for continuous variables and the \u03c7\u00b2 test ",
  "for categorical variables; p (age) is a global comparison across the ",
  "three age groups. Cardiometabolic indices are grouped as biochemical, ",
  "anthropometric and body-composition, and composite indices (formulas in ",
  "table 2). Fasting glucose was analysed only in participants ",
  "reporting \u22658 h of fasting. Abbreviations as in table 1."
)

cat("\n", supp_table_3_footnote, "\n")
